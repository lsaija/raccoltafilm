...
public class ExecuteSearchFilmServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (StringUtils.isNotBlank(registaIdParam)) {
			try {
				registaParam = registaService
						.caricaSingoloElemento(UtilityForm.parseIntegerRegistaIdFromString(registaIdParam));
			} catch (Exception e) {
		...
		Film example = new Film(titoloParam, genereParam, UtilityForm.parseDateArrivoFromString(dataPubblicazioneParam),
				UtilityForm.parseIntegerMinutiDurataFromString(minutiDurataParam), registaParam);
